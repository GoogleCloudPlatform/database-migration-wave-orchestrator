...
def test_rollback_require_args(client):
    pr = ProjectFactory()
    ...
    with patch('bms_app.services.operations.wave.RollbackService.run') as mock:
        req = client.post(f'/api/waves/{wave.id}/rollback', json={})
...
def test_specific_db_rollback(client):
    pr = ProjectFactory()
    wave = WaveFactory(project=pr)
    db_1 = SourceDBFactory(project=pr, wave=wave)
    db_2 = SourceDBFactory(project=pr, wave=wave)
    ...
    with patch('bms_app.services.operations.wave.RollbackService.run') as mock:
        req = client.post(f'/api/waves/{wave.id}/rollback',
                          json={'db_ids': [db_2.id]})

        assert req.status_code == 201
        assert req.json == {}

        mock.assert_called_with(1, [db_2.id])
