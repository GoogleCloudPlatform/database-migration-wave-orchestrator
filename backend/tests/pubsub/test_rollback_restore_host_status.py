...
def test_si_rollback_restore_complete_status(client):
    """Test correct db status update DT_COMPLETE -> DEPLOYED."""
    pr = ProjectFactory()
...
def test_si_restore_failed_status(client):
    """Test correct db status update DT_COMPLETE -> DT_FAILED."""
    pr = ProjectFactory()
    db_1 = SourceDBFactory(project=pr, wave=None, status='DT_COMPLETE')
    bms_1 = BMSServerFactory(name='oracle-bms-vm1')
    map_1 = MappingFactory(source_db=db_1, bms=bms_1)
...
def test_rollback_restore_rac_all_hosts_complete(client):
    ...
    db_1 = SourceDBFactory(
        project=pr,
        wave=None,
        status=SourceDBStatus.DT_COMPLETE,
        db_type=SourceDBType.RAC
    )
    bms_1 = BMSServerFactory(name='oracle-bms-vm1')
    bms_2 = BMSServerFactory(name='oracle-bms-vm2')
