...
from bms_app.services.control_node import RollbackConrolNodeService
...
def test_e2_mediun_type(zmock, cr_inst_mock, client):
    operation = OperationFactory(operation_type=OperationType.ROLLBACK)

    RollbackConrolNodeService.run(
    ...
    assert cr_inst_mock.call_args.kwargs['machine_type'] == 'e2-medium'
