...
import {dataMountPurposeOptions , BaseOptions, redundancyOptions, asmDiskOptionSelect, auSizeOptions, errorInfo}  from './config-editor.data';
import { ConfigEditor, DataMountValues, Node } from "@app-interfaces/configEditor";
...
export class ConfigEditorComponent implements OnInit, OnDestroy {
    ...
    private snackBar: MatSnackBar)
  {
    ...
    this.configEditorForm = this.formBuilder.group({
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.queryParams.subscribe((params) => {
      ...
      this.mappingService.getMappingsByDbId(params.databaseId).subscribe((resp) => {
        if (resp.data){
          ...
          this.configEditorForm.get('db_params_values')?.get('db_name')?.setValue(this.mappings.db_name);
          this.targetService.getTargetsProject(this.mappings.bms[0].id).subscribe(targets => {
            ...
            this.configEditorService.getConfigEditor(params.databaseId).subscribe( (resp:ConfigEditor) => {
              this.fillForm(resp);
              this.initValueMiscConfigValues(this.mappings as Mapping);
    ...
    this.subscriptions.push(
      this.configEditorForm.get('data_mounts_values')?.valueChanges.pipe(pairwise()).subscribe(([prevValue,nextValue]: [DataMountValues[], DataMountValues[]]) => {
        ...
        const oracleRoot = this.configEditorForm?.get('misc_config_values')?.get('oracle_root');
    ...
    this.subscriptions.push(
      this.configEditorForm.get('asm_config_values')?.valueChanges.subscribe(value => {
        this.selectedDevicesForAsmConfigValues = [];

        if (value[0]?.diskgroup) {
          this.configEditorForm?.get('rac_config_values')?.get('dg_name')?.setValue(value[0]?.diskgroup);
        }
  ...
  initRedundancy(oracle_version: string) {
    if (parseFloat(oracle_version.slice(0,4)) < 12.2) {
      this.redundancyOptions = this.redundancyOptions.filter(item => {
        return !item.is_new_version
      })
    }
  }
  ...
  async onSubmit() {
    ...
    if (await this.customValidateForm(this.configEditorForm)) {
      if (!this.currentProjectId) return;
      this.configEditorForm.get('is_configured')?.setValue(true);
      this.configEditorService.createConfigEditorsingleInstance(this.configEditorForm.value, this.db_id)
        .subscribe(() => {
  ...
  saveDraft() {
    this.prepareForm();
    if (!this.currentProjectId) return;
    this.configEditorForm.get('is_configured')?.setValue(false);
    this.configEditorService.createConfigEditorsingleInstance(this.configEditorForm.value, this.db_id)
      .subscribe(() => {
  ...
  get ldspFaDevices () {
    return this.configEditorForm.get('data_mounts_values') as FormArray
  }

  get asmFaData () {
    return this.configEditorForm.get('asm_config_values') as FormArray
  }

  get racNodes(): FormArray {
    return this.configEditorForm.get('rac_config_values')?.get('rac_nodes') as FormArray;
  }
  ...
  addDevice(): void {
    ...
    this.devices = this.configEditorForm.get('data_mounts_values') as FormArray;
  ...
  addAsmData() {
    ...
    this.asm_config_values = this.configEditorForm.get('asm_config_values') as FormArray;
  ...
  addDeviceAsmConf(index: number) {
    ...
    this.disks.push(ConfigEditorComponent.crateAsmDisk());
  }
  ...
  validateSwapCheckbox(): void {
    const swapDevice = this.ldspFaDevices.controls.find((device) => device.get('purpose')!.value === 'swap');

    if (swapDevice && this.configEditorForm.get('misc_config_values')?.get('is_swap_configured')?.value) {
      this.configEditorForm.get('misc_config_values')?.get('is_swap_configured')?.setErrors({'swapIsAlreadyConfigured': true})
      this.openSnackBar('Swap is already configured', 'OK');
    } else if (swapDevice && !this.configEditorForm.get('misc_config_values')?.get('is_swap_configured')?.value) {
      this.configEditorForm.get('misc_config_values')?.get('is_swap_configured')?.setErrors(null);
      swapDevice.get('purpose')?.setErrors(null);
    } else {
      this.configEditorForm.get('misc_config_values')?.get('is_swap_configured')?.setErrors(null);
    }
  }
  ...
  changeRol() {
    ...
    const grid_user = this.configEditorForm.get('install_config_values')?.get('grid_user');
    const grid_group = this.configEditorForm.get('install_config_values')?.get('grid_group');
    if (rolValue){
      grid_user?.enable();
      grid_user?.setValue('grid');
      grid_group?.enable();
      grid_group?.setValue('oinstall');
    }else{
      grid_user?.disable()
      grid_user?.setValue('');
      grid_group?.disable()
  ...
  getRolValue(): boolean {
    return this.configEditorForm.get('misc_config_values')?.get('role_separation')?.value
  }

  removeAsmDisk(formGroup: number, position: number) {
    if (this.asmFaDevices(formGroup).length == 1)
      return;
    this.asmFaDevices(formGroup).removeAt(position)
  }
  ...
  private diskSizeValidator(): errorInfo[] {
    const fg = this.configEditorForm;
   let errorAmount: errorInfo[] = [];
    fg.value.asm_config_values.forEach((element: any) => {
      const au_size = parseInt(element.au_size);
      let disc_size = 0;
      element.disks.forEach((d: any) => {
        disc_size += parseInt(d.size);
      });
      if(au_size !== null && disc_size !== null && au_size*4000000 < disc_size) {
        errorAmount.push({error: true, value: au_size*4})
      }
    });
    return errorAmount;
  }
